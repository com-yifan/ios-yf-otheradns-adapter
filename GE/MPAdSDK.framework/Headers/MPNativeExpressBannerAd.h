...
@class MPNativeExpressBannerAd;

NS_ASSUME_NONNULL_BEGIN

@protocol MPNativeExpressBannerAdDelegate <NSObject>
@optional

///  广告加载成功
///  在此方法中调用 showAdFromView:frame 方法
- (void)mp_nativeExpressBannerAdDidLoad:(MPNativeExpressBannerAd *)bannerAd;

/// 广告加载失败
- (void)mp_nativeExpressBannerAd:(MPNativeExpressBannerAd *)bannerAd didLoadFailWithError:(NSError *_Nullable)error;

/// 广告将要曝光
- (void)mp_nativeExpressBannerAdWillBecomVisible:(MPNativeExpressBannerAd *)bannerAd;

/// 广告曝光失败
- (void)mp_nativeExpressBannerAdBecomVisibleFail:(MPNativeExpressBannerAd *)bannerAd error:(NSError *_Nullable)error;

/// 广告被点击
- (void)mp_nativeExpressBannerAdDidClick:(MPNativeExpressBannerAd *)bannerAd;
 
/// 广告被关闭
- (void)mp_nativeExpressBannerAdClosed:(MPNativeExpressBannerAd *)bannerAd;

/// 广告详情页给关闭
- (void)mp_nativeExpressBannerAdViewDidCloseOtherController:(MPNativeExpressBannerAd *)bannerAd;

@end

@interface MPNativeExpressBannerAd : NSObject
...
/// 【必选参数】 广告的代理
@property(nonatomic,weak)id<MPNativeExpressBannerAdDelegate> delegate;
